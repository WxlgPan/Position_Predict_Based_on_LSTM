...
class Predict_via_LSTM_model:
    ...
    def predict_and_draw(self,userid, test_num=6, pred_num=1,draw_pic=True):
        per_num = pred_num
        data_all = pd.read_csv('./Geolife Trajectories 1.3/Data/'+userid+'/Trajectory/20081023025304.plt', sep=',',header=6)
        data_all = data_all.iloc[-2 * (test_num + per_num):-1 * (test_num + per_num),0:2].values
        print(data_all)
        ...
        # #归一化
        normalize = np.load("./npy/traj_model_trueNorm.npy")
        data = self.NormalizeMultUseData(data, normalize)

        model = keras.models.load_model("./model/traj_model_120.h5")
...
if __name__ == '__main__':
    ...
    trained_model.predict_and_draw(userid='000',test_num=6,pred_num=1)